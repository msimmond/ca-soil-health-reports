...
# Use absolute paths since Quarto runs from a different working directory
app_root <- here::here()
...
if (!is.null(params$grouping_var) && params$grouping_var != "" && params$grouping_var %in% names(producer_samples)) {
  grower_summary <- producer_samples |>
    ...
    dplyr::summarize(
      ...
      .by = c(measurement_group, measurement, abbr)  # Keep abbr!
    ) |>
    dplyr::mutate(`Field or Average` = "Your Fields")
} else {
...
# Safety net in case abbr is still missing:
if ("abbr" %in% names(grower_summary) && any(is.na(grower_summary$abbr))) {
  grower_summary <- grower_summary |>
    dplyr::left_join(
      dictionary |>
        dplyr::select(measurement_group, column_name, abbr),
      by = c("measurement_group", "measurement" = "column_name")
    )
}

# Process grower summary data

grower_summary <- grower_summary |>
  dplyr::mutate(`Field or Average` = dplyr::case_when(
    .data[["Field or Average"]] == "one Average \n(1 Fields)" ~ "Your Fields Average",
    .data[["Field or Average"]] == "two Average \n(1 Fields)" ~ "Your Fields Average",
    TRUE ~ .data[["Field or Average"]]
  ))
...
# Data processing for producer samples

# Get individual field data - we need to see what field IDs actually exist
producer_table <- producer_samples |>
  ...
  dplyr::summarize(
    n = dplyr::n(),
    value = mean(value, na.rm = TRUE),
    .by = c(measurement_group, abbr, `Field or Average`, Texture, measurement)
  ) |>
  dplyr::mutate(
    # Create dynamic labels with field counts for all grouping values
    `Field or Average` = glue::glue("{`Field or Average`}\nAverage\n({n} Fields)")
  )
...
# Combine data sources
            ...
            df_table <- dplyr::bind_rows(
              ...
              dplyr::mutate(
                ...
                `Field or Average` = as.character(`Field or Average`)  # Convert glue to character
              )

# If any abbr is NA, repair from dictionary via measurement
if (any(is.na(df_table$abbr))) {
  df_table <- df_table |>
    dplyr::left_join(
      dictionary |>
        dplyr::select(measurement_group, column_name, abbr),
...
# Final guard: drop rows that still have NA abbr (prevents a literal "NA" column)
df_table <- df_table |>
  dplyr::filter(!is.na(abbr))

# Remove any remaining duplicates by taking the first occurrence
df_table <- df_table |>
  dplyr::distinct(measurement_group, abbr, `Field or Average`, .keep_all = TRUE)
```
...
groups <- df_table |>
  ...
  purrr::map(\(x) {
    # Check if texture column exists (case-insensitive), if not use only "Field or Average"
    id_cols <- intersect(c("Field or Average", "Texture", "texture"), names(x))
    if (length(id_cols) == 0) id_cols <- "Field or Average"
    ...
    # Normalize case on the ID column
    nm <- names(result)
    if ("texture" %in% nm && !"Texture" %in% nm) {
      names(result)[nm == "texture"] <- "Texture"
    }
    # If somehow both exist, drop the lower-case one (or coalesce if needed)
    if (all(c("texture","Texture") %in% names(result))) {
      result$Texture <- dplyr::coalesce(result$Texture, result$texture)
      result <- dplyr::select(result, -texture)
    }
    
    # If a "NA" column still appears, drop it
    result <- result[, names(result) != "NA", drop = FALSE]
    
    result
  })

# remove Texture from all printable tables except Physical
tables <- purrr::imap(groups, \(x, group_name) {
  # Keep Texture only for Physical group
  if (group_name == "Physical") {
    return(x)  # Keep Texture for Physical
  } else {
    # Remove Texture from other groups
    dropme <- intersect(c("Texture","texture"), names(x))
    if (length(dropme)) dplyr::select(x, -all_of(dropme)) else x
  }
})
...
```{r make-flextables}
# Align headers and tables to prevent dimension mismatches
# Prune empty groups first (avoids pmap index errors)
keep <- vapply(tables, \(x) is.data.frame(x) && nrow(x) > 0 && ncol(x) > 1, logical(1))
tables  <- tables [keep]
headers <- headers[names(tables)]

# Align headers ↔ columns for each group (incl. Carbon)
for (grp in names(tables)) {
  # actual ID columns present for this group's table
  id_cols <- intersect(c("Field or Average", "Texture"), names(tables[[grp]]))
  data_cols <- setdiff(names(tables[[grp]]), id_cols)

  # keep only header rows that we actually have as data columns
  hdr_data <- headers[[grp]] |>
    dplyr::filter(.data$abbr %in% data_cols) |>
    dplyr::mutate(abbr = factor(abbr, levels = data_cols)) |>
    dplyr::arrange(abbr) |>
    dplyr::mutate(dplyr::across(everything(), as.character)) |>
    dplyr::distinct(key, .keep_all = TRUE)

  # build mapping rows for ID columns (empty unit row)
  hdr_ids <- NULL
  if (length(id_cols)) {
    hdr_ids <- tibble::tibble(
      abbr = id_cols,
      key  = id_cols,
      unit = rep("", length(id_cols))
    )
  }

  # final header mapping in the exact column order of the table
  header_full <- dplyr::bind_rows(hdr_ids, hdr_data)
  ordered_cols <- c(id_cols, as.character(hdr_data$abbr))
  tables[[grp]] <- tables[[grp]][, ordered_cols, drop = FALSE]

  # save back for make_ft()
  headers[[grp]] <- header_full
}

# Ensure Physical table column order is stable
if ("Physical" %in% names(tables)) {
  want <- c("Field or Average", "Texture", "Sand", "Silt", "Clay")
  have <- names(tables$Physical)
  front <- intersect(want, have)
  rest  <- setdiff(have, front)
  tables$Physical <- tables$Physical[, c(front, rest), drop = FALSE]
}
...
plot_list <- df_plot |>
  ...
  purrr::map(\(group) {
    if (!is.null(params$grouping_var) &&
      ...
      legend_levels <- c(producer_groups, "Other Fields")
      ...
      p <- set_dynamic_scales(
        ...
        scale_alpha_manual(
          values = stats::setNames(c(rep(1, length(producer_groups)), 0.8), legend_levels),
          limits = legend_levels
        )
