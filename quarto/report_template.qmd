...
if (!is.null(params$grouping_var) && params$grouping_var != "" && params$grouping_var %in% names(producer_samples)) {
  ...
  n_fields <- producer_samples |>
    dplyr::filter(!is.na(.data[[params$grouping_var]])) |>
    dplyr::distinct(sample_id, .data[[params$grouping_var]]) |>
    dplyr::count(.data[[params$grouping_var]], name = "n_fields")
  
  grower_summary <- producer_samples |>
    dplyr::filter(!is.na(.data[[params$grouping_var]])) |>
    dplyr::summarize(
      value = mean(value, na.rm = TRUE),
      .by = c(measurement_group, abbr, .data[[params$grouping_var]])
    ) |>
    dplyr::left_join(n_fields, by = params$grouping_var) |>
    dplyr::mutate(
      `Field or Average` = glue::glue("{.data[[params$grouping_var]]}\nAverage\n({n_fields} Fields)")
    ) |>
...
groups <- df_table |>
  ...
  purrr::map(\(x) {
    # Check if texture column exists, if not use only "Field or Average"
    id_cols <- if ("texture" %in% names(x)) {
