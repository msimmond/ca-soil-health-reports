...
# Resolve app root robustly so it works locally and on ShinyApps.io
qmd_dir  <- normalizePath(dirname(knitr::current_input(dir = TRUE)))
app_root <- normalizePath(file.path(qmd_dir, ".."))

src <- function(...) {
  p1 <- file.path(app_root, ...)
  p2 <- file.path(qmd_dir, ...)
...
# Set reference document path for Word output (resolve relative to app root)
reference_doc <- file.path(app_root, "inst", "templates", "report_template.docx")
```
...
# Project Average from all results (unchanged)
field_summary <- results_long |>
  dplyr::summarise(
    value = mean(value, na.rm = TRUE),
    .by = c(measurement_group, abbr)
...
if ("texture" %in% names(data) || "Texture" %in% names(data)) {
  ...
  if (has_group) {
    texture_by_group <- data |>
      ...
      dplyr::summarise(
        Texture = if (dplyr::n_distinct(texture_upper) == 1) {
          texture_upper[1]
        } else {
